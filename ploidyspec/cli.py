...
def add_common_args(p):
    p.add_argument("--manifest", required=True, help="TSV: <fasta_path>\\t<hap_label|AUTO> per line")
    ...
    p.add_argument("--min-len", type=int, default=1_000_000, help="minimum sequence length to treat as chromosome-scale (default 1e6)")
    p.add_argument("--chrom-regex", action="append", default=None,
    ...
    p.add_argument("--hap-regex", default=None,
    ...
    p.add_argument("--threads", type=int, default=os.cpu_count() or 4, help="parallel workers (default: all cores)")
    p.add_argument("--samtools", default=None, help="path to samtools binary or its containing dir")
    p.add_argument("--fastk-dir", default=None, help="dir containing FastK/Logex/Histex binaries")
...
def cmd_prepare(args):
    ...
    prepare(args.manifest, args.outdir, samtools_bin, args.min_len, args.chrom_regex, args.hap_regex)
...
def cmd_matrix(args):
    ...
    seq_tsv = os.path.join(args.outdir, "sequences.tsv")
    if not os.path.exists(seq_tsv):
        raise SystemExit(f"{seq_tsv} not found -- run the `prepare` and `kmers` stages first")
    compute_matrix(seq_tsv, args.outdir, logex_bin, histex_bin, args.threads)
    log(f"wrote {os.path.join(args.outdir, 'whole_chrom_distance_matrix.csv')} and "
...
def cmd_windowed(args):
    ...
    compute_windowed(seq_tsv, args.outdir, samtools_bin, fastk_bin, logex_bin, histex_bin, args.k, args.window, step, args.threads)
    log(f"wrote per-chromosome windowed_chrNN.tsv/.png, windowed_all.tsv and windowed_genome_overview.png in {args.outdir}")
...
def cmd_homeologs(args):
    ...
    if not os.path.exists(seq_tsv) or not os.path.exists(matrix_csv):
        raise SystemExit(f"need sequences.tsv and whole_chrom_distance_matrix.csv -- run `prepare`, `kmers`, `matrix` first")
    run_homeolog_detection(seq_tsv, args.outdir)
    log(f"wrote {os.path.join(args.outdir, 'homeolog_pairs.tsv')} and homeolog_pairs.png")
...
def cmd_windowed_homeologs(args):
    ...
    with open(pairs_tsv) as f:
        pairs = [(int(row["chrom_a"].replace("chr", "")), int(row["chrom_b"].replace("chr", ""))) for row in csv.DictReader(f, delimiter="\t")]
    if not pairs:
        raise SystemExit(f"{pairs_tsv} has no candidate pairs -- nothing to do")
    step = args.step or args.window
    compute_windowed_homeologs(seq_tsv, args.outdir, pairs, samtools_bin, fastk_bin, logex_bin, histex_bin, args.k, args.window, step, args.threads)
    log(f"wrote windowed_chrAAxBB.tsv/.png per pair, windowed_homeologs_all.tsv and windowed_homeologs_overview.png in {args.outdir}")
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="ploidyspec", description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    sub = parser.add_subparsers(dest="stage", required=True)

    p_prep = sub.add_parser("prepare", help="build sequences.tsv from a manifest of haplotype assemblies")
    ...
    p_matrix = sub.add_parser("matrix", help="whole-chromosome all-vs-all k-mer distance matrix")
    add_common_args(p_matrix)
    p_matrix.set_defaults(func=cmd_matrix)

    p_win = sub.add_parser("windowed", help="sliding-window k-mer divergence between haplotype copies of each chromosome")
    add_common_args(p_win)
    p_win.add_argument("--window", type=int, default=250_000, help="window size in bp (default 250000)")
    p_win.add_argument("--step", type=int, default=None, help="step size in bp (default: same as --window, i.e. tumbling windows)")
    p_win.set_defaults(func=cmd_windowed)

    p_all = sub.add_parser("all", help="run prepare -> kmers -> matrix -> windowed in sequence")
    add_common_args(p_all)
    p_all.add_argument("--window", type=int, default=250_000, help="window size in bp (default 250000)")
    p_all.add_argument("--step", type=int, default=None, help="step size in bp (default: same as --window, i.e. tumbling windows)")
    ...
    p_win_homeo.add_argument("--window", type=int, default=250_000, help="window size in bp (default 250000)")
    p_win_homeo.add_argument("--step", type=int, default=None, help="step size in bp (default: same as --window)")
